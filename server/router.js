 ...
 */
 router.get("/goods/item/insertTbItem", (req, res) => {
  ...
  const sql = "insert into project(title, image, sellPoint, price, cid, category, num, barcode,status,updated, descs, paramsInfo) values (?,?,?,?,?,?,?,?,?,?,?,?)"
  var arr = [title, image, sellPoint, price, cid, category, num, barcode,status,updated, descs, paramsInfo];
  sqlFn(sql, arr, result => {
      if (result.affectedRows > 0) {
          res.send({
              status: 200,
