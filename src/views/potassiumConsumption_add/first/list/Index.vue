<template>
    <div>
        ...
        <!-- <div>
            <el-breadcrumb separator-class="el-icon-arrow-right" style="margin-top: 10px; margin-bottom: 5px;margin-left: 8px; color: black; font-size: 15px; font-weight: bold;">
                <el-breadcrumb-item>碘酸钾岗位记录</el-breadcrumb-item>
        ...
        <div class="maincontrol">
            <el-table :header-cell-style="{ background: '#f2f2f2' }" border :data="tableData" size="" style="width: 99%; margin-bottom: 20px">
                ...
                <el-table-column prop="peiR" label="配制人/检测人" min-width="100" :show-overflow-tooltip="false">
                    <!-- <template slot-scope="scope">
                        <el-input type="textarea" :autosize="{ minRows: 1, maxRows: 5 }" v-model="scope.row.peiR" class="input-text" @blur="blurEvent(scope.row, scope.$index)">
                        </el-input>
                    </template> -->
                ...
                <el-table-column prop="jiandur" label="监督人" min-width="100" :show-overflow-tooltip="false">
                    <!-- <template slot-scope="scope">
                        <el-input type="textarea" :autosize="{ minRows: 1, maxRows: 5 }" v-model="scope.row.jiandur" class="input-text" @blur="blurEvent(scope.row, scope.$index)">
                        </el-input>
                    </template> -->
                ...
                <el-table-column  label="设备检查记录（一次/周）" min-width="300" :show-overflow-tooltip="false">
                    ...
                    <el-table-column prop="jiandur1" label="监督人" min-width="100" :show-overflow-tooltip="false">
                        <!-- <template slot-scope="scope">
                            <el-input type="textarea" :autosize="{ minRows: 1, maxRows: 5 }" v-model="scope.row.jiandur1" class="input-text" @blur="blurEvent(scope.row, scope.$index)">
                            </el-input>
                        </template> -->
                ...
                <el-table-column prop="nongdu" label="碘酸钾溶液浓度（%）" min-width="60" :show-overflow-tooltip="false">
                    <template slot-scope="scope">
                        <el-input type="textarea" :autosize="{ minRows: 1, maxRows: 5 }" v-model="scope.row.nongdu" class="input-text" @blur="blurEvent(scope.row, scope.$index)">
                        </el-input>
                    </template>
                </el-table-column>

                <el-table-column label="检验监督人" min-width="300" :show-overflow-tooltip="false">
                    <el-table-column prop="huayanR" label="（化验员）" min-width="100" :show-overflow-tooltip="false">
                        <!-- <template slot-scope="scope">
                            <el-input type="textarea" :autosize="{ minRows: 1, maxRows: 5 }" v-model="scope.row.huayanR" class="input-text" @blur="blurEvent(scope.row, scope.$index)">
                            </el-input>
                        </template> -->

                        <template slot-scope="scope">
                            <el-select v-model="scope.row.huayanR" placeholder="请选择" @change="onChange(scope.row, scope.$index)">
                                <el-option v-for="item in optionss" :key="item.value" :label="item.label" :value="item.value">
                                </el-option>
                            </el-select>
                        </template>
    ...
    export default {
        created() {
            this.potassiumConsumptionListadd({})
            this.fetchOptions();
        },
        data() {
            return {
        ...
        if(tab.name === 'first11') {
            ...
            onSubmit() {
                ...
                this.potassiumConsumptionListadd(start, end)
            ...
            async potassiumConsumptionListadd(start, end) {
                console.log("start", start)
                console.log("end", end)
                let res = await this.$api.potassiumConsumptionListadd({
                    start: start,
                    end: end
                });
            ...
            deleteRow(index, rows) {
                ...
                this.$confirm('此操作将永久删除该信息, 是否继续?', '提示', {
                    ...
                    // 用户确认删除，执行删除操作和刷新 UI  
                    this.deletepotassiumConsumptionadd(id);
                    ...
                    this.potassiumConsumptionListadd(this.start, this.end);
                });
            },
            async deletepotassiumConsumptionadd(id) {
                let res = await this.$api.deletepotassiumConsumptionadd({
                    id: id
                });
                console.log(res)
                if (res.status == 200) {
            ...
            async updatePotassiumConsumptionadd(params) {
                let res = await this.$api.updatePotassiumConsumptionadd(params);
                ...
                try {
                    ...
                    const promises = this.tableData.map(async (item) => {
                        ...
                        if (item.isNew) {
                            ...
                            return this.$api.addPotassiumConsumptionadd({
                            ...
                            return this.$api.updatePotassiumConsumptionadd({
                                id: item.id,
                                biaoTime: item.biaoTime,
                                pihao: item.pihao,
                                peijiliang: item.peijiliang,
                    ...
                    await this.potassiumConsumptionListadd(this.start, this.end); // 强制重新加载数据
