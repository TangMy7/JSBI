    ...
    <div>
        ...
        <div>
            <el-breadcrumb separator-class="el-icon-arrow-right"
                ...
                <el-breadcrumb-item>报警编辑</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="header">
            <div class="group">
                <el-button type="warning" @click="addComment">报警添加</el-button>
        ...
        <el-table ref="multipleTable" border stripe :data="tableData" height="700" tooltip-effect="dark"
            ...
            <el-table-column prop="namee" label="名字" min-width="130" :show-overflow-tooltip="false">
                <template slot-scope="scope">
                    <el-input type="textarea" :autosize="{ minRows: 1, maxRows: 5 }" v-model="scope.row.namee"
                        class="input-text" @blur="blurEvent(scope.row, scope.$index)">
                    </el-input>
                </template>
            </el-table-column>

            <el-table-column prop="PointId" label="点位" min-width="150" :show-overflow-tooltip="false">
                <template slot-scope="scope">
                    <el-input type="textarea" :autosize="{ minRows: 1, maxRows: 5 }" v-model="scope.row.PointId"
                        class="input-text" @blur="blurEvent(scope.row, scope.$index)">
                    </el-input>
                </template>
            </el-table-column>

            <el-table-column  label="可能的原因" min-width="330" :show-overflow-tooltip="false">

                <el-table-column prop="possibleCause" label="大于" min-width="330" :show-overflow-tooltip="false">
                    <template slot-scope="scope">
                        <el-input type="textarea" :autosize="{ minRows: 1, maxRows: 5 }" v-model="scope.row.possibleCause"
                            class="input-text" @blur="blurEvent(scope.row, scope.$index)">
                        </el-input>
                    </template>
                </el-table-column>

                <el-table-column prop="naocan1" label="小于" min-width="330" :show-overflow-tooltip="false">
                    <template slot-scope="scope">
                        <el-input type="textarea" :autosize="{ minRows: 1, maxRows: 5 }" v-model="scope.row.naocan1"
                            class="input-text" @blur="blurEvent(scope.row, scope.$index)">
                        </el-input>
                    </template>
                </el-table-column>
            ...
            <el-table-column  label="解决措施" min-width="560" :show-overflow-tooltip="false">
                <el-table-column prop="Solution" label="大于" min-width="560" :show-overflow-tooltip="false">
                    <template slot-scope="scope">
                        <el-input type="textarea" :autosize="{ minRows: 1, maxRows: 5 }" v-model="scope.row.Solution"
                            class="input-text" @blur="blurEvent(scope.row, scope.$index)">
                        </el-input>
                    </template>
                </el-table-column>
                <el-table-column prop="naocan2" label="小于" min-width="560" :show-overflow-tooltip="false">
                    <template slot-scope="scope">
                        <el-input type="textarea" :autosize="{ minRows: 1, maxRows: 5 }" v-model="scope.row.naocan2"
                            class="input-text" @blur="blurEvent(scope.row, scope.$index)">
                        </el-input>
                    </template>
                </el-table-column>
            ...
            <!-- <el-table-column prop="handling" label="处理情况">
                <template slot-scope="scope">
                    <el-select v-model="scope.row.handling" placeholder="请选择" @blur="blurEvent(scope.row, scope.$index)"
                        :disabled="scope.row.disabledPost">
            ...
            <el-table-column prop="unit" label="单位">
                <template slot-scope="scope">
                    <el-select v-model="scope.row.unit" placeholder="请选择" @blur="blurEvent(scope.row, scope.$index)"
                        :disabled="scope.row.disabledPost">
            ...
            <el-table-column prop="Typee" label="类型">
                <template slot-scope="scope">
                    <el-select v-model="scope.row.Typee" placeholder="请选择" @blur="blurEvent(scope.row, scope.$index)"
                        :disabled="scope.row.disabledPost">
...
export default {
    ...
    methods: {
        ...
        addComment() {
            this.changeComment({})
            this.$router.push('/superVip/AddAlarm')
        },
        onSubmit() {
            this.alarmList(this.currentPage)
        ...
        async alarmList(currentPage) {
            let res = await this.$api.alarmList({
                page: currentPage
            });
    ...
    row.normalRange = row.normalRange.replace(/，/g, ',');
        ...
        async alarmDel(id) {
            let res = await this.$api.alarmDel({
                id: id
            });
            console.log(res)
            if (res.status == 200) {
        ...
        async updatealarm(params) {
            let res = await this.$api.updatealarm(params);
            console.log(res)
        },
        async Submit() {
            try {
                const updatePromises = this.currentIds.map(id => {
                    ...
                    return this.updatealarm({
                        ...
                        naocan1: item.naocan1,
                        naocan2: item.naocan2,

                    });
                });

                await Promise.all(updatePromises);
                this.$message.success('更新报警成功');
                this.currentIds = []; // 清空
                await this.alarmList(this.currentPage);
            } catch (error) {
                this.$message.error('更新报警失败');
            }
        }
