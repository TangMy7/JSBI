        ...
        <el-row :gutter="20">
            <el-col :span="6">
                ...
                <div class="grid-content bg-purple">
                    <el-form-item label="经办人">
                        <el-select 
                            v-model="formInline.handoverPerson" 
                            placeholder="请选择经办人" 
                            filterable 
                            allow-create>
                            <el-option 
                                v-for="item in formInline.handoverPersonOptions" 
                                :key="item.value" 
                                :label="item.label" 
                                :value="item.value">
                            </el-option>
...
export default {
    ...
    created() {
        ...
        this.inputTimeDisplay = `${year}-${month}-${day}`;
        // this.inputTimeDisplay = moment().format('YYYY-MM-DD');
    ...
    try {
        ...
        // 过滤出 role 为 "班长" 的数据，用于班长下拉框
        this.options3 = response.data
            .filter(item => item.role === '班长') // 筛选出 role 为 "班长" 的数据
            .map(item => ({
                value: item.username, // 设置下拉选项的值
                label: item.username // 设置下拉选项的显示文本
            }));

        // 经办人下拉框显示所有数据
        this.formInline.handoverPersonOptions = response.data.map(item => ({
            value: item.username, // 设置下拉选项的值
            label: item.username // 设置下拉选项的显示文本
        }));
    } catch (error) {
        ...
        async measureAdd(params) {
            try {
                ...
                // 对于不管是500 还是400 都直接到catch了，没有经过上面else,估计是post原因，之前get都没见过这种情况
                this.$message({
                    message: '请勿重复添加数据！',
                    type: 'error',
                    duration: 5000
