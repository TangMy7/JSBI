      ...
      <div class="sycm">
        ...
        <ul class="clearfix">
          ...
          <li>
            ...
            <div class="row-values">
              <span @click="editValue('today_luhao')" :class="{ editable: true }">{{today_luhao + today_taoxi}}</span>
              <span @click="editValue('month_luhao')" :class="{ editable: true }">{{month_luhao + month_taoxi}}</span>
              <span @click="editValue('year_luhao')" :class="{ editable: true }">{{year_luhao +year_taoxi}}</span>
            </div>
          </li>
        </ul>
      </div>
...
export default {
  ...
  methods: {
    ...
    // 添加数据获取方法
    fetchData() {
      axios.get('http://172.32.12.100:5000/api/data2/')
        .then(response => {
          ...
          if (data) {
            ...
            this.today_taoxi = data.today_taoxi || 0;
            this.month_taoxi = data.month_taxoi || 0;
            this.year_taoxi = data.year_taoxi || 0;
  ...
  methods: {
    ...
    validateAndProcessData(data) {
      ...
      const requiredFields = [
        ...
        'today_luhao', 'month_luhao', 'year_luhao','today_taoxi','month_taoxi','year_taoxi'
      ];
    ...
    async fetchData() {
      ...
      try {
        const response = await axios.get('http://172.32.12.100:5000/api/data2/', {
          timeout: 20000 // 5秒超时
        });
        ...
        if (this.validateAndProcessData(data)) {
          ...
          this.today_taoxi=this.processAbsoluteValue(data.today_taoxi);
          this.month_taoxi=this.processAbsoluteValue(data.month_taoxi);
          this.year_taoxi=this.processAbsoluteValue(data.year_taoxi);
