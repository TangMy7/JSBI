<template>
  <div class="boxall" style="height: calc(24% - .10rem)">
    <div>
      <div class="alltitle">消耗</div>
      <div class="sycm">
        <!-- 表头 -->
        ...
        <ul class="clearfix">
          ...
          <li>
            <span class="row-title">汽耗(T)</span>
            <div class="row-values">
          ...
          <li>
            <span class="row-title">卤耗(m³)</span>
            <div class="row-values">
...
export default {
  ...
  methods: {
    ...
    // 添加数据获取方法
    fetchData() {
      axios.get('http://172.32.12.100:9072/api/data7/')
        .then(response => {
...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        const response = await axios.get('http://172.32.12.100:9072/api/data7/', {
          timeout: 2000 // 5秒超时
        });
