  ...
  <div class="boxall" style="height: calc(20% - .10rem)">
    <div>
      ...
      <div class="sycm">
        <ul class="clearfix">
          <li>
            <h2 @click="editValue('today_chanliang')" :class="{ editable: true }">{{ today_chanliang }}</h2>
            <span>日产量</span>
          </li>
          <li>
            <h2 @click="editValue('month_chanliang')" :class="{ editable: true }">{{ month_chanliang }}</h2>
            <span>当月累计</span>
          </li>
          <li>
            <h2 @click="editValue('year_chanliang')" :class="{ editable: true }">{{ year_chanliang }}</h2>
            <span>本年度累计</span>
          </li>
        </ul>
      </div>
...
.sycm li h2{ font-size:.6rem; color: #c5ccff; font-family: electronicFont; cursor: pointer;}
