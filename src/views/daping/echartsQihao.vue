    ...
    <div class="avg-popup" v-if="showAvgData">
      <div class="avg-popup-content">
        <div class="avg-item" v-for="(item, index) in avgData" :key="index">
          ...
          <span class="avg-value">{{item.avg_qiHao_unit.toFixed(2)}}</span>
        </div>
      </div>
    </div>
  </div>
  ...
  methods: {
    ...
    processChartData(rawData) {
      ...
      rawData.forEach(item => {
        ...
        consumptionData[className][dateKey] = item.qiHao_unit || 0;
      });
