...
export default {
  ...
  const option = {
    ...
    legend: {
      data: ['本月', this.currentType === 'year' ? '去年同月' : '上月'],
      textStyle: { color: '#fff' }
    },
    grid: {
      left: '0%',
    ...
    series: [
      {
        ...
        data: base.map((val, i) => {
          ...
          return {
            value: val,
            xAxisIndex: isProduction ? 1 : 0,
            label: {
              show: true,
              position: 'right',
              formatter: rateLabels[i],
              color: '#fff',
              fontSize: 12
            }
          }
      ...
      {
        name: this.currentType === 'year' ? '去年同月' : '上月',
        type: 'bar',
        data: compare.map((val, i) => {
          const isProduction = i >= 4
          return {
            value: val,
            xAxisIndex: isProduction ? 1 : 0
...
export default {
  ...
  // 能耗: 前4项有值，后2项为null；产量: 前4项为null，后2项有值
  ...
  },
  {
    ...
    axisLabel: {
      ...
      formatter: function(value) {
        // 只显示最小和最大刻度
        return (value === 0 || value === 40000) ? value : '';
      }
    },
    ...
    series: [
      ...
      },
      {
        ...
        type: 'bar',
        xAxisIndex: 0,
        data: compareEnergy.map(val => ({ value: val })),
        itemStyle: { color: '#6be6c1' }
      },
      {
        name: '本月产量',
