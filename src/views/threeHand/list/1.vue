    ...
    <div>
        ...
        <div>
            <el-breadcrumb separator-class="el-icon-arrow-right" style="margin-top: 10px; margin-bottom: 15px;margin-left: 8px; color: black; font-size: 15px; font-weight: bold;">
                <el-breadcrumb-item>电气卤数据管理</el-breadcrumb-item>
                <el-breadcrumb-item>电气卤数据修改</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="header">
            <div class="form">
            ...
            <div class="group">
                <!-- <el-button type="warning" @click="addComment">添加电气卤数据</el-button> -->
        ...
        <el-table ref="multipleTable" border stripe :data="tableData" height="700" tooltip-effect="dark" @selection-change="selsChange" style="width: 100%; margin-top: 30px" class="custom-table">
            ...
            <el-table-column prop="sub" label="电气卤添加时间段" min-width="300">
                <template slot-scope="scope">
            ...
            <el-table-column prop="classes" label="班别" min-width="200">
                <template slot-scope="scope">
                    <el-select v-model="scope.row.classes" placeholder="请选择" @change="onChange(scope.row, scope.$index)" :disabled="!canEdit(scope.row.inputTime)">
            ...
            <el-table-column prop="dianHao" label="电耗" min-width="300">
                <template slot-scope="scope">
                    <el-input type="number" v-model="scope.row.dianHao" class="input" @blur="blurEvent(scope.row,scope.$index)" readonly></el-input>
                </template>
            </el-table-column>
            <el-table-column prop="qiHao" label="气耗" min-width="300">
                <template slot-scope="scope">
                    <el-input type="number" v-model="scope.row.qiHao" class="input" @blur="blurEvent(scope.row,scope.$index)" readonly></el-input>
                </template>
            </el-table-column>
            <el-table-column prop="luHao" label="卤耗" min-width="300">
                <template slot-scope="scope">
                    <el-input type="number" v-model="scope.row.luHao" class="input" @blur="blurEvent(scope.row,scope.$index)" readonly></el-input>
                </template>
            </el-table-column>
    ...
    export default {
        ...
        methods: {
            ...
            onSubmit() {
                ...
                let end = dayjs(this.date).add(23, 'hour').format("YYYY-MM-DD HH:mm:ss") // 24
                console.log('日期', this.date)
                this.start = start
                this.end = end
                this.startTimestamp = dayjs(this.date).format("YYYY-MM-DD")
                this.endTimestamp = dayjs(this.date).add(23, 'hour').format("YYYY-MM-DD") //24
                this.threeHandList(start, end)
